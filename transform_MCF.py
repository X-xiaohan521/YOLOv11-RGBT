...
def copy_and_modify_layers(infrared_backbone_path, visible_backbone_path, mcf_skeleton_path, output_model_path):
    ...
    # 加载三个输入模型
    infrared_backbone = load_model(infrared_backbone_path)
    visible_backbone = load_model(visible_backbone_path)
    mcf_skeleton = load_model(mcf_skeleton_path)
